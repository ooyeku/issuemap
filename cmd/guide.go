...
func showGuide() {
	...
	// Step 17: Advanced Search & Filtering
	printStep("17", "Advanced Search & Filtering")
	fmt.Println("Find issues quickly with powerful query syntax:")
	fmt.Println()
	printCommand("issuemap search \"type:bug priority:high\"")
	printCommand("issuemap search \"assignee:alice status:open\"")
	printCommand("issuemap search \"created:>2024-01-01 AND labels:urgent\"")
	printCommand("issuemap search \"updated:<7d OR milestone:v1.0\"")
	fmt.Println()
	printCommand("# Save frequently used searches")
	printCommand("issuemap search save \"urgent-bugs\" \"type:bug priority:high status:open\"")
	printCommand("issuemap search run urgent-bugs")
	fmt.Println()
	printNote("Query syntax supports field filters, date ranges, and boolean operators")
	fmt.Println()

	// Step 18: Time Tracking & Estimation
	printStep("18", "Time Tracking & Estimation")
	fmt.Println("Track time spent on issues and estimate work:")
	fmt.Println()
	printCommand("# Estimate work before starting")
	printCommand("issuemap estimate ISSUE-001 4h")
	printCommand("issuemap estimate ISSUE-002 2d")
	fmt.Println()
	printCommand("# Track time as you work")
	printCommand("issuemap start ISSUE-001           # Start timer")
	printCommand("issuemap stop                      # Stop current timer")
	printCommand("issuemap log ISSUE-001 2h30m       # Log time manually")
	fmt.Println()
	printCommand("# Generate reports and analytics")
	printCommand("issuemap report time               # Time tracking summary")
	printCommand("issuemap report velocity           # Team velocity")
	printCommand("issuemap report burndown           # Sprint burndown")
	fmt.Println()
	printNote("Time tracking helps with project planning and performance analysis")
	fmt.Println()

	// Step 19: File Attachments
	printStep("19", "File Attachments")
	fmt.Println("Attach files and manage compression:")
	fmt.Println()
	printCommand("issuemap attach ISSUE-001 screenshot.png      # Single file")
	printCommand("issuemap attach ISSUE-001 *.log               # Multiple files")
	printCommand("issuemap attach ISSUE-001 doc.pdf --description \"Requirements\"")
	fmt.Println()
	printCommand("# Manage compression")
	printCommand("issuemap compress --enable --level 6           # Enable compression")
	printCommand("issuemap compress --stats                      # Show statistics")
	printCommand("issuemap compress --run-batch                  # Compress existing files")
	fmt.Println()
	printNote("Attachments support compression and deduplication for efficient storage")
	fmt.Println()

	// Step 20: Archive & Data Management
	printStep("20", "Archive & Data Management")
	fmt.Println("Archive old issues and manage storage:")
	fmt.Println()
	printCommand("issuemap archive --closed --older-than 6m     # Archive old closed issues")
	printCommand("issuemap archive --list                        # List archives")
	printCommand("issuemap archive --search \"bug fix\"            # Search archived issues")
	printCommand("issuemap archive --restore ISSUE-001           # Restore from archive")
	fmt.Println()
	printCommand("# Storage management")
	printCommand("issuemap storage status                        # Storage usage")
	printCommand("issuemap cleanup                               # Clean temporary files")
	printCommand("issuemap restore --list                        # List backups")
	fmt.Println()
	printNote("Regular archiving keeps your active project lean while preserving history")
	fmt.Println()

	// Step 21: Bulk Operations & Deduplication
	printStep("21", "Bulk Operations & Deduplication")
	fmt.Println("Efficiently manage multiple issues:")
	fmt.Println()
	printCommand("# Bulk operations")
	printCommand("issuemap bulk --status open --set-priority high")
	printCommand("issuemap bulk --assignee alice --add-labels urgent")
	printCommand("issuemap bulk --type bug --assign bob")
	fmt.Println()
	printCommand("# Remove duplicates")
	printCommand("issuemap dedup                                 # Find duplicates")
	printCommand("issuemap dedup --threshold 0.8                 # Set similarity threshold")
	printCommand("issuemap dedup --merge ISSUE-001 ISSUE-002     # Merge specific issues")
	fmt.Println()
	printNote("Bulk operations and deduplication help maintain data quality at scale")
	fmt.Println()
	...
	// Step 22: Issue Dependencies & Blocking
	printStep("22", "Issue Dependencies & Blocking")
	fmt.Println("Model complex relationships between issues:")
	fmt.Println()
	printCommand("# Create dependencies")
	printCommand("issuemap depend ISSUE-001 ISSUE-002 --type blocks")
	printCommand("issuemap depend ISSUE-003 ISSUE-004 --type requires")
	fmt.Println()
	printCommand("# Visualize dependency relationships")
	printCommand("issuemap deps --graph              # Show dependency graph")
	printCommand("issuemap deps --blocked            # List blocked issues")
	printCommand("issuemap list --blocked            # Filter blocked in list")
	fmt.Println()
	printCommand("# Analyze impact and validate")
	printCommand("issuemap deps --impact ISSUE-001   # Impact analysis")
	printCommand("issuemap deps --validate           # Check for circular deps")
	printCommand("issuemap deps --stats              # Dependency statistics")
	fmt.Println()
	printNote("Dependencies help manage complex projects with interconnected tasks")
	fmt.Println()
	...
	printTip("Save frequently used search queries for quick access")
	printTip("Use time estimation to improve project planning accuracy")
	printTip("Start timers when beginning work for automatic tracking")
	printTip("Model dependencies between issues to manage complex projects")
	printTip("Regularly validate dependency graphs to avoid circular issues")
	printTip("Use blocking analysis to identify project bottlenecks")
	fmt.Println()
	...
	fmt.Printf("%-25s %s\n", "issuemap delete", "Delete issues")
	fmt.Printf("%-25s %s\n", "issuemap template", "Manage issue templates")
	fmt.Printf("%-25s %s\n", "issuemap branch", "Create Git branch for issue")
	fmt.Printf("%-25s %s\n", "issuemap search", "Advanced search & filtering")
	fmt.Printf("%-25s %s\n", "issuemap estimate", "Estimate issue work time")
	fmt.Printf("%-25s %s\n", "issuemap start", "Start time tracking timer")
	fmt.Printf("%-25s %s\n", "issuemap stop", "Stop active timer")
	fmt.Printf("%-25s %s\n", "issuemap log", "Log time manually")
	fmt.Printf("%-25s %s\n", "issuemap attach", "Attach files to issues")
	fmt.Printf("%-25s %s\n", "issuemap compress", "Manage attachment compression")
	fmt.Printf("%-25s %s\n", "issuemap report", "Generate time/velocity reports")
	fmt.Printf("%-25s %s\n", "issuemap velocity", "Track team velocity")
	fmt.Printf("%-25s %s\n", "issuemap burndown", "Generate burndown charts")
	fmt.Printf("%-25s %s\n", "issuemap depend", "Manage issue dependencies")
	fmt.Printf("%-25s %s\n", "issuemap deps", "Visualize dependency graph")
	fmt.Printf("%-25s %s\n", "issuemap bulk", "Bulk operations on issues")
	fmt.Printf("%-25s %s\n", "issuemap dedup", "Remove duplicate issues")
	fmt.Printf("%-25s %s\n", "issuemap archive", "Archive old issues")
	fmt.Printf("%-25s %s\n", "issuemap restore", "Restore from backups")
	fmt.Printf("%-25s %s\n", "issuemap cleanup", "Clean temporary files")
	fmt.Printf("%-25s %s\n", "issuemap storage", "Manage storage")
	fmt.Printf("%-25s %s\n", "issuemap sync", "Sync with Git")
	fmt.Printf("%-25s %s\n", "issuemap server", "Manage local API server")
	fmt.Printf("%-25s %s\n", "issuemap web", "Open web interface")
	fmt.Printf("%-25s %s\n", "issuemap history", "View change history")
	fmt.Printf("%-25s %s\n", "issuemap logs", "View system logs")
	fmt.Printf("%-25s %s\n", "issuemap global", "Cross-project operations")
	fmt.Printf("%-25s %s\n", "issuemap version", "Show version information")
